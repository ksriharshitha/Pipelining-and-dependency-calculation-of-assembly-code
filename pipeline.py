	...
	for i in list:
		...
		if(y>2):
			...
			if((i[0]!="li") and (i[1]!="$v0")):
				...
				out_list=[]
	return fresh_list
'''def check(to_check,list1):
	flag=0
	for i in list1:
		if to_check in i:
			flag=1
	return flag'''
def two_out_lists(check_1,check_2):
	flag=0
	if(check_1==check_2):
		flag=1
	return flag
def check_instructionsand(to_check):
...
def check_without_bypassing_dependency_write_after_write_output_reg_read_after(list):
	x=len(list)
	for i in range(x-4):
		if((check_instructionsand(list[i][0][0])==3) or (check_instructionsand(list[i][0][0])==4) or (check_instructionsand(list[i][0][0])==5)):
			for j in range(i+1,i+2):
				if ((list[i][1][0]==list[j][1][0]) or (two_out_lists(list[i][1][0],list[j][2])==1)):
		...
		if((list[i][0][0]=="lw") or (list[i][0][0]=="la")):
			...
			for j in range(x-1,x):
				if ((list[i][1][0] in list[j][2])):
					print(list[i][1][0],list[j][2],i+1,j+1,"=>dependency(-r-a-w-)")					
def main():
	ins=SplitFileInput("code.asm")
	comm=splitinstructionss(ins)
	print("ASSUMPTION: 'It takes an entire cycle to read and write'")
	finalList=subListSplitting(comm)
	print("------------------------------------------------------------------")
	for i in range(len(finalList)):
		print(finalList[i]," ===> ",i+1)
	print("----------read-after-write-dependencies-without----------")
	check_without_bypassing_dependency_read_after_write(finalList)
	'''print("----------write-after-write-dependencies-without---------")
	check_without_bypassing_dependency_write_after_write_output_reg_read_after(finalList)'''
	print("----------read-after-write-dependencies-with----------")
	check_with_bypassing_dependency_read_after_write(finalList)
	print("------------------------------------------------------------")
	print("There are no write after write or write after read dependencies in in-order pipelinining")
main()
